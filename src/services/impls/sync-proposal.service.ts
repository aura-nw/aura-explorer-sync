    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if(item.pro_status === CONST_PROPOSAL_STATUS.PROPOSAL_STATUS_VOTING_PERIOD){
            const paramsTally = `cosmos/gov/v1beta1/proposals/${item.proposal_id}/tally`;
             proposalTally = await this._commonUtil.getDataAPI(this.api, paramsTally);
          }
          //create proposal
          let proposal = SyncDataHelpers.makerProposalData(item, proposalTally);          
